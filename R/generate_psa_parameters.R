...
generate_psa_params <- function(n_sim = n_sim, seed = seed, n_pop = n_pop, scenario = scenario,
  ...
  df_psa_params <- data.frame(
    ...
    p_fent_exp_2020 = rbeta(n_sim, shape1 = df_fentanyl["shape1", "pe"], shape2 = df_fentanyl["shape2", "pe"]),
    ...
    p_NX_2020 = rbeta(n_sim, shape1 = df_naloxone["shape1", "pe"], shape2 = df_naloxone["shape2", "pe"]), #Hold NX at 2020 levels
    ...
    ### Costs ###
    # Treatment Costs
    c_BUP_TX  = rgamma(n_sim, shape = df_costs["shape", "BUP_TX"], scale = df_costs["scale", "BUP_TX"]),
    c_MET_TX  = rgamma(n_sim, shape = df_costs["shape", "MET_TX"], scale = df_costs["scale", "MET_TX"]),
