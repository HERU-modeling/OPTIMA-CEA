...
outcomes <- function(l_params_all, 
  ...
  with(as.list(l_params_all), {
    ...
    for (i in 1:nrow(l_out_markov$m_M_trace)){
      ...
      if(i == 1){
        m_HRU_costs[i, ODF & NI]  <- m_HRU_costs[i, ODF & NI]
        m_HRU_costs[i, ODF & INJ] <- m_HRU_costs[i, ODF & INJ]
      } else{
        m_HRU_costs[i, ODF & NI]  <- (m_HRU_costs[i, ODF & NI] - m_HRU_costs[i - 1, ODF & NI])
        m_HRU_costs[i, ODF & INJ] <- (m_HRU_costs[i, ODF & INJ] - m_HRU_costs[i - 1, ODF & INJ])
      }
